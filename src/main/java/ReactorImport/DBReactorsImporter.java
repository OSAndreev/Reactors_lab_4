        ...
        switch(group) {
            case "Страна":
                query = "SELECT country as Страна, year, " +
                        "ROUND(SUM(consumption), 2) AS yearLoadFactor FROM (" +
                        "SELECT country, year, " +
                        "CASE WHEN year != firstGridConnection " +
                        "THEN (((thermalCapacity/burnup) * LoadFactor.loadFactor) / 100000*365)*3 " +
                        "ELSE (((thermalCapacity/burnup) * LoadFactor.loadFactor) / 100000*365) " +
                        "END AS consumption " +
                        "FROM ReactorsPris " +
